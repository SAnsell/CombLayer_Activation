  ...
  if (!cx.fail())
    {
      ...
      if (!cx.fail() && (xc=='d' || xc=='D'))    // special case : 1D/d-9 etc
	{
	  int prev=xc;
	  A[static_cast<size_t>(xpt)]='e';
	  if (sectionMCNPX(A,out)) return 1;
	  A[static_cast<size_t>(xpt)]=static_cast<char>(prev);
	  return 0;
	}
...
pairSplit(const std::string& Item,
          const std::string& divider,
          const bool revDir)
  /*!
    Simple algorith to split the string about the last
    \param Item :: Split item 
    \param divider :: item to divide on
    \param revDir :: reverse direction
  ...
  if (divider.empty())
  ...
  else  // divider valid
    {
      const std::string::size_type pos =
        (revDir) ? Item.rfind(divider) : Item.find(divider);
      
      if (pos != std::string::npos)
        {
          Out.first=Item.substr(0,pos);
          Out.second=Item.substr(pos+1);
        }
      else
        Out.second=Item;
    }
